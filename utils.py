...
def plot_solution(B_true, B_est, B_processed, save_name=None):
    ...
    """
    fig, axes = plt.subplots(figsize=(10, 3), ncols=3, dpi=600)
    ...
    # plt.show()
    # outputs_dir = './results/'
    # output_path = os.path.join(outputs_dir, save_name)
    # if save_name is not None:
    #     fig.savefig(output_path, bbox_inches='tight')


def plot_dysolution(B, A, save_name=None):
    fig, axes = plt.subplots(figsize=(12, 3), ncols=A.shape[0]+1, dpi=600)
    im = axes[0].imshow(B, cmap='RdBu', interpolation='none',
                        vmin=-2.25, vmax=2.25)
    for i in range(len(B)):
        for j in range(len(B)):
            if B[i, j] !=0:
                axes[0].text(j, i, round(B[i, j],2),
                   ha="center", va="center", color="black", fontsize=8)

    axes[0].set_title("W (Intra-slice)", fontsize=13)
    axes[0].tick_params(labelsize=13)

    for i in range(A.shape[0]):
        im = axes[i+1].imshow(A[i], cmap='RdBu', interpolation='none',
                            vmin=-2.25, vmax=2.25)
        axes[i+1].set_title("A%d(Inter-slice)"%(i+1), fontsize=13)
        axes[i+1].tick_params(labelsize=13)
        axes[i+1].axes.yaxis.set_ticklabels([])
        for m in range(len(A[i])):
            for n in range(len(A[i])):
                if A[i][m, n] != 0:
                    axes[i+1].text(n, m, round(A[i][m, n], 2),
                                ha="center", va="center", color="black", fontsize=8)
    # Adjust space between subplots
    fig.subplots_adjust(wspace=0.1)
